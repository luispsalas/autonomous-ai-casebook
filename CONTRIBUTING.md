...
This casebook improves when readers point out what it got wrong, what it missed, and where a better source exists. This page explains how to do that and what standard a change has to meet.

---

## How to contribute: issues only

**Open an issue; don't open a pull request.** Every change to an entry is made by the maintainer after checking it against the sourcing standard below. That keeps each entry's reasoning and revision history in one consistent voice, and it means a correction is verified before it is published, not after.
...
**Link the source itself.** A claim needs a link to where it was published, not a description of it. If the source is paywalled, say so; if it is only available in an archive, link the archived copy.
...
**Sources in other languages are welcome.** Link the original. If the publisher offers an official translation, link that too.
...
Each issue gets read against the sources it links. If a correction holds up, the entry is revised, and the revision is recorded in the entry's own changelog, so readers can see what changed and why. If it doesn't hold up, the issue says why.
